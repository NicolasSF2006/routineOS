...
export function RoutineBlockRow({
  ...
  isCurrent,
  onOpenStudy,
}: RoutineBlockRowProps) {
  const meta = BLOCK_TYPE_META[block.type]
  const canOpenStudy =
    isCurrent && isStudyActionBlock(block) && typeof onOpenStudy === "function"
  ...
  return (
    ...
    >
      <div
        className={cn("h-10 w-1.5 shrink-0 rounded-full", meta.dot)}
        aria-hidden="true"
      />

      <div className="flex w-16 shrink-0 flex-col font-mono text-sm sm:w-20">
        <span className="text-foreground font-semibold">{block.startTime}</span>
        <span className="text-muted-foreground text-sm">{block.endTime}</span>
      </div>
