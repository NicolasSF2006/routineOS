...
export function DataBackupSettingsCard() {
  ...
  const handleImportBackup = async (file: File | null) => {
    ...
    const confirmed = window.confirm(
      "Importar este backup vai substituir rotina, histórico, configurações, tema, tela atual e status do tutorial. Deseja continuar?",
    )
  ...
  const handleResetAllData = () => {
    const confirmed = window.confirm(
      "Resetar tudo vai apagar rotina personalizada, histórico de estudos, sons importados, metas, tema, tela atual e status do tutorial. Esta ação não pode ser desfeita sem backup. Deseja continuar?",
    )
        ...
        <div className="grid gap-3 rounded-xl border border-border bg-muted/30 p-4">
          <div className="flex flex-col gap-1">
            ...
            <span className="wrap-break-word text-sm text-muted-foreground">
              Inclui rotina, histórico, configurações, sons importados, tema, última tela acessada e status do tutorial.
            </span>
          </div>
